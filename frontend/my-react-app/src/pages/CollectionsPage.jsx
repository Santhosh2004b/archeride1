...
function CollectionsPage() {
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] bg-gray-50 text-brandDark px-4 sm:px-6 py-4 relative">
      ...
      <h1 className="font-marcellus text-2xl sm:text-3xl font-bold text-gray-900 mb-4">Collections</h1>
      {mode === "view" && (
        <>
                    ...
                    <tr key={c.id} className="border-b border-gray-100 hover:bg-gray-50/80">
                      <td className="px-3 sm:px-4 py-2">
                        <button
                          type="button"
                          onClick={() => {
                            window.location.href = `${window.location.pathname}?mode=edit&id=${c.id}`;
                          }}
                      ...
                      <td className="px-3 sm:px-4 py-2 text-[11px] sm:text-sm whitespace-nowrap">{c.status}</td>
                    </tr>
                  ))}
                  {rows.length === 0 && (
                    <tr>
      ...
      {mode === "edit" && editId && (
        <div className="mt-4 rounded-xl bg-white border border-gray-200 shadow-sm p-5">
          <h3 className="font-urbanist font-semibold text-sm mb-4">Edit Collection</h3>
          {/* ...form fields here... */}
          <button

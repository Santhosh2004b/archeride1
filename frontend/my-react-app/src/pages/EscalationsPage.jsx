...
function EscalationsPage() {
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] bg-gray-50 text-brandDark px-4 sm:px-6 py-4 relative">
      ...
      <h1 className="font-marcellus text-2xl sm:text-3xl font-bold text-gray-900 mb-4">Escalations</h1>
      {mode === "view" && (
        <>
              ...
              <table className="min-w-full text-left text-xs sm:text-sm font-urbanist">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    ...
                    <th className="px-3 sm:px-4 py-2.5 text-[11px] sm:text-xs font-semibold text-brandMuted uppercase tracking-wide whitespace-nowrap">Reported Date</th>
                  </tr>
                </thead>
                <tbody>
                  {rows.map((e) => (
                    <tr key={e.id} className="border-b border-gray-100 hover:bg-gray-50/80">
                      <td className="px-3 sm:px-4 py-2">
                        <button
                          type="button"
                          onClick={() => {
                            window.location.href = `${window.location.pathname}?mode=edit&id=${e.id}`;
                          }}
                      ...
                      <td className="px-3 sm:px-4 py-2 text-[11px] sm:text-sm whitespace-nowrap">{e.escalation_id}</td>
                      <td className="px-3 sm:px-4 py-2 text-[11px] sm:text-sm whitespace-nowrap">{e.manual_project_id}</td>
                      <td className="px-3 sm:px-4 py-2 text-[11px] sm:text-sm whitespace-nowrap">{e.status}</td>
                      <td className="px-3 sm:px-4 py-2 text-[11px] sm:text-sm whitespace-nowrap">{e.priority}</td>
                      <td className="px-3 sm:px-4 py-2 text-[11px] sm:text-sm whitespace-nowrap">{e.title}</td>
                      <td className="px-3 sm:px-4 py-2 text-[11px] sm:text-sm whitespace-nowrap">{e.reported_date}</td>
                    </tr>
                  ))}
                  {rows.length === 0 && (
                    <tr>
                      <td colSpan={7} className="px-3 sm:px-4 py-4 text-center text-xs sm:text-sm text-brandMuted">
